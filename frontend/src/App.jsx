...
function textareaStyle(t, height) {
  return {
    width: '100%', height, padding: '10px 2px', fontSize: 13,
...
function Chip({ t, color, children }) {
  ...
  return (
    <span style={{ display: 'inline-block', padding: '3px 10px', borderRadius: 6, fontSize: 12, fontWeight: 500, background: c.bg, color: c.text, border: `1px solid ${c.border}` }}>{children}</span>
  );
}
...
export default function App() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (data.is_complete) {
        setSession(prev => ({ ...prev, assessments: { ...(prev.assessments || {}), [activeSkill]: data.assessment_data } }));
        const remaining = session.skills_to_assess?.filter(s => s !== activeSkill && !(session.assessments?.[s]));
        if (remaining?.length > 0) {
          setMessages(prev => [...prev, { role: 'system', content: `✓ ${activeSkill} assessed. Assess more or generate your plan.` }]);
          setStep('gaps');
  ...
  const resetAll = () => {
    setStep('input'); setSession(null); setMessages([]); setLearningPlan(null);
    setActiveSkill(null); setJdText(''); setResumeText(''); setResumeFileName('');
    setError(''); setAssessmentDone(false);
  };
      ...
      <div style={{ minHeight: '100vh', background: t.bg, fontFamily: fonts.body, color: t.text, transition: 'background 0.3s, color 0.3s' }}>
        ...
        <header style={{ position:'sticky', top:0, zIndex:100, background:t.headerBg, backdropFilter:'blur(14px)', borderBottom:`1px solid ${t.border}`, padding:'0 28px', height:56, display:'flex', alignItems:'center', justifyContent:'space-between', transition:'background 0.3s,border-color 0.3s' }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <span style={{ fontFamily:fonts.display, fontWeight:800, fontSize:16, letterSpacing:'-0.02em' }}>
              SkillGap<span style={{ color:t.purple }}>AI</span>
            </span>
          </div>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            {step !== 'input' && (
              <button onClick={resetAll} style={{ fontSize:12, color:t.textSecondary, background:'none', border:'none', padding:'4px 10px', borderRadius:6 }}>
                ← Start over
              </button>
            )}
            {aiMode === 'demo' && (
        ...
        <div style={{ display:'flex', minHeight:'calc(100vh - 56px)' }}>
              ...
              <div style={{ maxWidth:680, margin:'0 auto' }}>
                <div className="fade-up" style={{ marginBottom:36 }}>
                  ...
                  <h1 style={{ fontFamily:fonts.display, fontSize:'clamp(26px,4vw,42px)', fontWeight:800, lineHeight:1.1, letterSpacing:'-0.03em', marginBottom:14 }}>
                    Bridge the gap between<br/><span style={{ color:t.purple }}>resume</span> and reality
                  </h1>
                  <p style={{ fontSize:15, color:t.textSecondary, lineHeight:1.7, maxWidth:500 }}>
                    Paste a job description and resume. Our AI conducts a conversational assessment to verify actual proficiency — then builds a personalized learning plan.
                  </p>
                </div>
                ...
                <div className="fade-up-1" style={{ display:'flex', flexDirection:'column', gap:18 }}>
                  ...
                  <InputCard t={t} label="Candidate Resume" icon="📄">
                    <div style={{ display:'flex', gap:10, marginBottom:10, alignItems:'center' }}>
                      <label style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'6px 13px', borderRadius:8, border:`1px solid ${t.border}`, background:t.bgSecondary, fontSize:13, color:t.textSecondary, cursor:'pointer', fontWeight:500, transition:'all 0.15s' }}>
            ...
            {step === 'assessing' && (
              <div style={{ maxWidth:660, margin:'0 auto', display:'flex', flexDirection:'column', height:'calc(100vh - 120px)' }}>
                <div className="fade-up" style={{ display:'flex', alignItems:'center', gap:8, marginBottom:18 }}>
                  ...
                  {['voice','text'].map(mode => (
                    <button key={mode} onClick={() => setInterviewMode(mode)} style={{ padding:'5px 14px', borderRadius:6, fontSize:12, fontWeight:600, background:interviewMode===mode?t.text:'transparent', color:interviewMode===mode?t.accentText:t.textSecondary, border:`1px solid ${interviewMode===mode?t.text:t.border}`, transition:'all 0.15s', letterSpacing:'0.02em' }}>
                      {mode === 'voice' ? '🎙 Voice' : '💬 Text'}
                    </button>
                  ))}
                  <div style={{ marginLeft:'auto', fontSize:13, color:t.textSecondary }}>
                    Skill: <span style={{ color:t.purple, fontWeight:600 }}>{activeSkill}</span>
                  </div>
                </div>
          ...
          {sidebarVisible && (
            <aside style={{ width:340, flexShrink:0, borderLeft:`1px solid ${t.border}`, background:t.bgCard, padding:'24px 20px', overflowY:'auto', transition:'background 0.3s,border-color 0.3s' }}>
              ...
              <GapReport gaps={session?.gaps} assessments={session?.assessments} skillsToAssess={session?.skills_to_assess} onStartAssessment={handleStartAssessment} activeSkill={activeSkill} t={t} />
              {step !== 'plan' && (
                <button onClick={handleGeneratePlan} disabled={loading} style={{ marginTop:16, width:'100%', padding:'11px', borderRadius:10, background:t.green, color:'#fff', border:'none', fontWeight:600, fontSize:13, cursor:'pointer', opacity:loading?0.5:1, transition:'all 0.15s' }}>
